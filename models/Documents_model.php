...
class Documents_model extends Model
{
    public function add_document($tmp_file, $directory_path, $document_name)
    {
        ...
        if (!move_uploaded_file($tmp_file, $directory_path ."/" .$document_name)) {
            throw new Exception('Le fichier n\'a pas pu etre ajouté sur le serveur');
        }
    }
    ...
    public function check_directory($directory_path)
    {
        if (!is_dir($directory_path)) {
            if (!mkdir($directory_path)) {
                throw new Exception('Le dossier n\'a pas pu etre crée.');
            }
        }
    }
    ...
    {
        if (!unlink("uploads/" .$numEvent ."/" .$document_name)) {
            throw new Exception('Le document n\'a pas pu etre supprimé du serveur');
        }
    }
